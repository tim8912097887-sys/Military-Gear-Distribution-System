...
class DatabaseServer {
  ...
  constructor() {
    ...
    subscribeShutdown(() => this.dbDisconnection());
  }
  ...
  public async testConnection() {
    try {
      ...
      logger.info('Database Connection: Verified successfully via test query.');
      client.release();
    } catch (error: any) {
      logger.error(`Database Connection: Failed to verify connection. Error: ${error}`);
